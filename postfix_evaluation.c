...
double pop(OperandStack *sp)
{
	if (isEmpty(sp))
	{
		printf("Stack Undrerflow, terminating program\n");
		exit(1);
	}
	return sp->item[sp->top--];
}

// returns 1 if the stack is empty, otherwise 1
...
double evalPostfix(char postfix[])
{
	...
	while(postfix[i] != '\0')
	{
		...
		if (token >= '0' && token <= '9') // 0 is askey value 48.
		{
			int v = token - '0'; // converting char to int 
			push(&stack, (double)v);
